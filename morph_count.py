...
while True:
    ...
    for s in result_json["return_object"]["sentence"]:
        ...
        # sentence에 포함된 morp 리스트를 반복하며 count 계산
        for m in s["morp"]:
            if m["type"]=="NNG" or m["type"]=="NNP":
                morph_key = m["lemma"] + "/" + m["type"]
                if morph_dic.get(morph_key):
                    morph_dic[morph_key] = morph_dic[morph_key] + 1
                else:
                    morph_dic[morph_key] = 1
...
print("\n# 형태소 빈도수")
f = open("morph_count.txt", 'w', encoding='utf-8')
for k in morph_dic.keys():
    s = k + "\t" + str(morph_dic[k])
    print(s)
    f.write(s+"\n")
f.close()
